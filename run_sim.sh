...
#     FEED_GT_SEMANTIC  FEED_GT_BBOX  FEED_GT_BBOX_ALL_FLOORS
...
# GT boxes are visualization-only, latched RViz layers. The host derives them from the semantic
# GLB; it does not send them to perception/admission. The ROS side publishes configured structural
# labels and actual objects on separate topics, with only the requested storey drawn by default.
# Set FEED_GT_BBOX_ALL_FLOORS=1 to draw every storey.
...
docker run --name "${GRAPH_API_CONTAINER_NAME:-graphapi_live}" --rm --entrypoint bash --gpus "${GRAPH_API_GPUS:-all}" --network=host \
  ...
  -e GRAPH_API_RUN_ID -e FEED_GT_SEMANTIC -e FEED_GT_BBOX -e FEED_GT_BBOX_ALL_FLOORS -e ROS_DOMAIN_ID \
