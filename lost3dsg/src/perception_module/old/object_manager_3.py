...
class RoomManager:
    ...
    def evaluate_scene(self, current_scene_objects, persistent_objects, encoded_image=None, vlm_callback=None):
        ...
        if any("door" in l for l in clean_labels):
            current_time = time.time()
            # COOLDOWN
            if (current_time - getattr(self, 'last_door_time', 0)) > 15.0:
                print(f"🚪 [DOOR DETECTED] Trigger attivato ISTANTANEAMENTE.")
                
                # 1. Salva la stanza VECCHIA con i suoi muri
                self.finalize_current_room(persistent_objects)

                # 2. Crea la stanza NUOVA
                self.room_counter += 1
                self.current_room_id = f"room_{self.room_counter}"
                self.init_room_node(self.current_room_id)
                
                # ---> AGGIUNGI QUESTA RIGA: SVUOTA I MURI VECCHI! <---
                self.current_room_walls = []
                # -----------------------------------------------------

                self.last_door_time = current_time
    ...
    def save_room_to_json(self, room_id, label, description, objects, walls):
        ...
        # Aggiorna o aggiungi in memoria
        ...
        # Sovrascrive sempre il file con lo stato attuale
        with open(json_path, "w") as f:
            json.dump(self.rooms, f, indent=4)

    def finalize_current_room(self, persistent_objects):
            room_id = self.current_room_id
            print(f"🛑 [FINALIZZAZIONE] Salvataggio stanza: {room_id} (Calcolo area demandato a Perception)")
            
            # Recupera gli oggetti della stanza
            room_objs = [o for o in persistent_objects if getattr(o, 'room_id', room_id) == room_id]
            room_labels = [o.label.split('#')[0].lower().replace(" ", "_") for o in room_objs]
            
            # ========================================================
            # LOGICA SEMANTICA E SALVATAGGIO JSON
            # ========================================================
            if room_labels:
                # 1. Chiama il VLM (OpenAI) per ottenere nome e descrizione
                semantic_name, description = self.ask_vlm_room_info(room_labels)
                
                # 2. Stampe di conferma
                print(f"🧠 [VLM] Stanza {room_id} classificata come: {semantic_name}")
                print(f"📝 [VLM] Descrizione: {description}")
                
                # 3. Aggiorna lo Scene Graph in memoria
                self.scene_graph[room_id]["semantic_label"] = semantic_name
                self.scene_graph[room_id]["description"] = description
                
                # 4. Finalmente, SALVA IL FILE JSON!
                self.save_room_to_json(
                    room_id=room_id, 
                    label=semantic_name, 
                    description=description, 
                    objects=self.scene_graph[room_id]["objects"], 
                    walls=self.current_room_walls
                )
            else:
                print(f"⚠️ Nessun oggetto trovato in {room_id}, impossibile interrogare il VLM.")
...
class ObjectManagerService(Node):
    ...
    def object_tracking_callback(self, request, response):
        ...
        # --- VALUTAZIONE STANZA PERIODICA O IMMEDIATA SE C'È UNA PORTA ---
        current_time = time.time()
        
        # 1. Controlliamo subito se nei nuovi oggetti c'è una porta
        door_detected_now = False
        for d in request.descriptions.descriptions:
            if "door" in d.label.lower():
                door_detected_now = True
                break
                
        # 2. Scatta se vediamo una porta, OPPURE se sono passati 5 secondi
        if door_detected_now or (current_time - getattr(self, 'last_room_check_time', 0)) > 5.0:
            if len(request.descriptions.descriptions) > 0:
                # --- NOVITÀ: Salvo il vecchio ID prima di valutare la scena ---
                old_room_id = self.room_manager.current_room_id
                
                # Il Room Manager pensa e decide se siamo in una nuova stanza
                self.room_manager.evaluate_scene(request.descriptions.descriptions, wm.persistent_perceptions)
                
                # --- NOVITÀ: Controllo se l'ID è cambiato dopo la valutazione ---
                if self.room_manager.current_room_id != old_room_id:
                    # Invia il segnale al nodo Perception!
                    from std_msgs.msg import String # (Solo per sicurezza, in caso manchi l'import in cima)
                    room_msg = String()
                    room_msg.data = self.room_manager.current_room_id
                    self.room_pub.publish(room_msg)
                    self.log_both('info', f"🚪 Cambio stanza! Inviato segnale a Perception per: {self.room_manager.current_room_id}")
                # --------------------------------------------------------------
    ...
    def walls_callback(self, msg):
        try:
            import json
            import math # Necessario per calcolare la lunghezza
            new_walls = json.loads(msg.data)
            
            # --- FILTRO LUNGHEZZA MINIMA ---
            # Imposta la lunghezza minima di un muro (es. 0.5 metri)
            MIN_WALL_LENGTH = 0.5 
            valid_walls = []
            
            for w in new_walls:
                if len(w) >= 4:
                    # w = [x_start, y_start, x_end, y_end]
                    length = math.hypot(w[2] - w[0], w[3] - w[1])
                    if length >= MIN_WALL_LENGTH:
                        valid_walls.append(w)
                        
            # Passiamo solo i muri che hanno superato il test
            self.room_manager.current_room_walls.extend(valid_walls)
        except Exception as e:
            print(f"Errore parsing muri: {e}")
...
def main(args=None):
    ...
    try:
        ...
        if hasattr(service_node, 'room_manager') and hasattr(service_node.room_manager, 'last_known_objects'):
            service_node.room_manager.finalize_current_room(service_node.room_manager.last_known_objects)
