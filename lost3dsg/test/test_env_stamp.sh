#!/usr/bin/env bash
# Runnable check for the environment stamping added to run_sim.sh.
#   ./test_env_stamp.sh
#
# Reads the REAL run_sim.sh rather than a copy — a test that restates the logic drifts
...
bash -n "$SRC" || fail "run_sim.sh does not parse"
...
# Extract between explicit markers in run_sim.sh. Two earlier versions guessed the boundary
...
[ -n "$(type -t _enc_rev)" ] || fail "_enc_rev not extracted from run_sim.sh"
