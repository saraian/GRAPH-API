...
OVERRIDES = {
    ...
    "merge": {"periodic_interval_s": 5.0,
              # EVENT-DRIVEN: one sweep within this many seconds of any world-model change
              # (admission, update, merge, box write, room change), none while idle. The
              # periodic timer stays on as a floor; _merge_call_lock serialises the two.
              # 1.0 s is a debounce, not a tuned value: the sweep costs 0.6 ms median.
...
def main():
    ...
    ap.add_argument("--variant", choices=["ms1"], default=None,
                    help="ms1: merge_min_consecutive 1 with merge_min_sightings_per_side 2 -- "
                         "the re-measurement requirement moved from the pair to the objects. "
                         "Output file gets the suffix _ms1; launch with VARIANT=ms1.")
    a = ap.parse_args()
    base = yaml.safe_load(open(TRACKED))
    over = copy.deepcopy(OVERRIDES)
    over["run"]["cap_min"] = a.cap_min
    if a.variant == "ms1":
        over.setdefault("association", {}).update(
            {"merge_min_consecutive": 1, "merge_min_sightings_per_side": 2})
    if a.engine:
    ...
    suffix = f"_{a.variant}" if a.variant else ""
    out = os.path.join(HERE, f"merge_debug_00824_{engine}{suffix}.yaml")
